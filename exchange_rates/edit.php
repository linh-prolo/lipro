...
$stmt = $conn->prepare("SELECT * FROM exchange_rates WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!verifyCsrfToken()) {
        ...
        if (empty($code) || $rv <= 0) {
            ...
            $stmt = $conn->prepare("UPDATE exchange_rates SET currency_code=?, currency_name=?, rate_to_vnd=?, effective_date=?, updated_by=? WHERE id=?");
            $stmt->bind_param("ssdsii", $code, $name, $rv, $date, $uid, $id);
            if ($stmt->execute()) {
                logActivity($conn, 'update', 'exchange_rates', "Cập nhật tỷ giá: $code");
                $stmt->close();
                $conn->close();
...
<head>
    ...
    <title>Sửa Tỷ giá - LIPRO LOGISTICS</title>
        ...
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="d-flex align-items-center gap-2 mb-3">
                    ...
                    <h5 class="mb-0"><i class="bi bi-pencil"></i> Sửa tỷ giá</h5>
                </div>
                        ...
                        <form method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="currency_code" class="form-control text-uppercase"
                                       required maxlength="10" value="<?php echo htmlspecialchars($_POST['currency_code']); ?>">
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="effective_date" class="form-control"
                                       value="<?php echo htmlspecialchars($_POST['effective_date'] ?? ''); ?>">
                            </div>
