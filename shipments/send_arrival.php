...
// ✅ MỚI - lấy đúng an_exchange_usd/eur, fallback nếu chưa set
...
// ============================================================
function buildArrivalXlsx($shipment, $foreign_charges, $domestic_charges, $usd_rate, $eur_rate): string {
    $C_RED  = 'C00000';
    $C_BLUE = '2F5496';
    $C_GRN  = '538135';
    ...
    // Helper vẽ bảng phí
    $drawTable = function(string $title, string $colorHex, array $charges) use ($sheet, &$r): void {
        $sheet->getRowDimension($r)->setRowHeight(18); $sheet->mergeCells("B{$r}:J{$r}");
        ...
        if (empty($charges)) {
            ...
            foreach ($charges as $i=>$c) {
                ...
                $sheet->getRowDimension($r)->setRowHeight(14);
                $cs = fn($align)=>['font'=>['size'=>8,'name'=>'Arial'],'fill'=>['fillType'=>Fill::FILL_SOLID,'startColor'=>['rgb'=>$bg]],'alignment'=>['horizontal'=>$align,'vertical'=>Alignment::VERTICAL_CENTER],'borders'=>['bottom'=>['borderStyle'=>Border::BORDER_HAIR,'color'=>['rgb'=>'DDDDDD']]]];
...
// ============================================================
function buildArrivalMailBody($shipment, $foreign_charges, $domestic_charges, $usd_rate, $eur_rate, $extra = ''): string {
    $hawb    = $shipment['hawb'] ?? '';
    $cd_no   = $shipment['customs_declaration_no'] ?? '';
    $cus     = $shipment['company_name'] ?? '';
    $arrival = !empty($shipment['arrival_date']) ? date('d/m/Y', strtotime($shipment['arrival_date'])) : '—';

    $makeRows = function($charges, $title) {
        if (empty($charges)) return '';
        $html = "<tr style='background:#1B3A6B;color:#fff;'><th colspan='7' style='padding:8px 12px;text-align:left;font-size:13px;'>{$title}</th></tr>
        <tr style='background:#f0f0f0;'>
            <th style='padding:6px 8px;border:1px solid #ccc;font-size:12px;'>Diễn giải</th>
            <th style='padding:6px 8px;border:1px solid #ccc;text-align:center;font-size:12px;'>Tiền tệ</th>
            <th style='padding:6px 8px;border:1px solid #ccc;text-align:right;font-size:12px;'>Đơn giá</th>
            <th style='padding:6px 8px;border:1px solid #ccc;text-align:center;font-size:12px;'>SL</th>
            <th style='padding:6px 8px;border:1px solid #ccc;text-align:right;font-size:12px;'>Thành tiền</th>
            <th style='padding:6px 8px;border:1px solid #ccc;text-align:right;font-size:12px;'>Quy đổi VND</th>
            <th style='padding:6px 8px;border:1px solid #ccc;text-align:right;font-size:12px;'>Tổng VND</th>
        </tr>";
        foreach ($charges as $i=>$c) {
            $bg = $i%2==0?'#fff':'#f8f9fa';
            $amount     = floatval($c['amount']     ?? 0);
            ...
            $html .= "<tr style='background:{$bg};'>
                <td style='padding:6px 8px;border:1px solid #dee2e6;font-size:12px;'>".htmlspecialchars($c['description']??'')."</td>
                <td style='padding:6px 8px;border:1px solid #dee2e6;text-align:center;font-size:12px;'>".htmlspecialchars($c['currency']??'USD')."</td>
                <td style='padding:6px 8px;border:1px solid #dee2e6;text-align:right;font-size:12px;'>".number_format(floatval($c['unit_price']??0),2,',','.')."</td>
                <td style='padding:6px 8px;border:1px solid #dee2e6;text-align:center;font-size:12px;'>".number_format(floatval($c['quantity']??1),2,',','.')."</td>
                <td style='padding:6px 8px;border:1px solid #dee2e6;text-align:right;font-size:12px;'>".($amount>0?number_format($amount,2,',','.'):'-')."</td>
                <td style='padding:6px 8px;border:1px solid #dee2e6;text-align:right;font-size:12px;'>".($amount_vnd>0?number_format($amount_vnd,0,',','.'):'-')."</td>
                <td style='padding:6px 8px;border:1px solid #dee2e6;text-align:right;font-weight:bold;color:#198754;font-size:12px;'>".($total_vnd>0?number_format($total_vnd,0,',','.'):'-')."</td>
            </tr>";
        }
        $total_vnd = array_sum(array_column($charges,'total_vnd'));
        $html .= "<tr style='background:#fff3cd;font-weight:bold;'>
            <td colspan='6' style='padding:7px 8px;border:1px solid #ccc;text-align:right;font-size:12px;'>TỔNG:</td>
            <td style='padding:7px 8px;border:1px solid #ccc;text-align:right;color:#dc3545;font-size:12px;'>".($total_vnd>0?number_format($total_vnd,0,',','.').' VND':'-')."</td>
        </tr>";
        return $html;
    };
...
<p style='margin:0;font-size:13px;'><strong>Thông báo hàng đến:</strong><span style='color:#C00000;font-weight:bold;font-size:14px;'>".($hawb?" HAWB: {$hawb}":'').($cd_no?" &nbsp;|&nbsp; TK: {$cd_no}":'')."</span></p></div>
<p style='margin:0 0 4px;font-size:14px;'><strong>Kính chào:</strong><span style='color:#1B3A6B;font-weight:bold;'> ".htmlspecialchars($cus)."</span></p>
...
<tr style='background:#f8f9fa;'><td style='padding:6px 12px;font-weight:bold;border-bottom:1px solid #dee2e6;'>MAWB:</td><td style='padding:6px 12px;border-bottom:1px solid #dee2e6;'>".htmlspecialchars($shipment['mawb']??'—')."</td></tr>
...
<tr style='background:#f8f9fa;'><td style='padding:6px 12px;font-weight:bold;border-bottom:1px solid #dee2e6;'>Số tờ khai:</td><td style='padding:6px 12px;border-bottom:1px solid #dee2e6;'>".htmlspecialchars($cd_no?:'—')."</td></tr>
<tr><td style='padding:6px 12px;font-weight:bold;'>Kho:</td><td style='padding:6px 12px;'>".htmlspecialchars($shipment['warehouse']??'—')."</td></tr>
</table>
...
<tr style='background:#1B3A6B;font-weight:bold;color:#fff;'><td colspan='6' style='padding:9px 8px;text-align:right;font-size:14px;'>TỔNG THANH TOÁN:</td><td style='padding:9px 8px;text-align:right;font-size:15px;color:#90ee90;'>".($grand>0?number_format($grand,0,',','.').' VND':'-')."</td></tr>
</table>
{$extra_html}
...
<head>
    ...
    <style>
        ...
        .section-hdr { background:linear-gradient(90deg,#1B3A6B,#2E75B6); color:#fff; padding:8px 14px; border-radius:6px 6px 0 0; font-weight:700; font-size:.9rem; }
    </style>
</head>
<body class="bg-light">
...
<div class="container-fluid mt-3 pb-5">
    ...
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Gửi Thông Báo Hàng Đến</li>
        </ol>
    </nav>
        ...
        <div class="col-xl-5 col-lg-6">
            <div class="card shadow-sm">
                <div class="card-header bg-info text-white py-2">
                    <h5 class="mb-0"><i class="bi bi-envelope-fill"></i> Gửi Thông Báo Hàng Đến - <?php echo htmlspecialchars($shipment['job_no']); ?></h5>
                </div>
                <div class="card-body">

                    <!-- Đã gửi trước -->
                    <?php if (!empty($shipment['arrival_notice_sent']) && $shipment['arrival_notice_sent'] == 'yes'): ?>
                    <div class="email-sent-banner">
                        <div class="d-flex align-items-center gap-2">
                            <i class="bi bi-envelope-check-fill text-success fs-4"></i>
                            <div>
                                <strong class="text-success">Đã gửi Arrival Notice trước đó!</strong>
                                <?php if (!empty($shipment['arrival_notice_sent_at'])): ?>
                                <br><small class="text-muted"><i class="bi bi-clock"></i> Lần gửi gần nhất: <strong><?php echo date('d/m/Y H:i', strtotime($shipment['arrival_notice_sent_at'])); ?></strong></small>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <div class="d-flex flex-wrap gap-2 mb-3">
                        ...
                        <span class="info-chip"><i class="bi bi-currency-dollar text-success"></i> <?php echo number_format($grand_total_vnd, 0, ',', '.'); ?> VND</span>
                    </div>
                    ...
                    <form method="POST" action="send_arrival.php?id=<?php echo $id; ?>" id="sendForm">
                        ...
                        <div class="mb-3">
                            ...
                            <!-- Arrival Notice Excel — tự động -->
                            <div class="att-item mb-2">
                                <i class="bi bi-file-earmark-excel text-success fs-5"></i>
                                <div class="flex-grow-1">
                                    <div class="fw-bold small">ArrivalNotice_<?php echo $shipment['job_no']; ?>_<?php echo date('Ymd'); ?>.xlsx</div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <button type="submit" class="btn btn-warning btn-lg px-4" id="sendBtn"
                                    onclick="return confirm('Đã gửi Arrival Notice trước đó!\nBạn có chắc muốn GỬI LẠI không?')">
                                <i class="bi bi-send-fill"></i> Gửi lại Email
                            </button>
                            <?php else: ?>
                            <button type="submit" class="btn btn-info btn-lg px-4" id="sendBtn">
                        ...
                        <div class="p-3">
                            <table class="table table-sm table-bordered">
                                ...
                                <tr><th class="bg-light">Đính kèm:</th>
                                    <td>
                                        <div><i class="bi bi-file-earmark-excel text-success"></i> ArrivalNotice_<?php echo $shipment['job_no']; ?>_<?php echo date('Ymd'); ?>.xlsx <span class="badge bg-success">Auto</span></div>
                                        <?php foreach ($saved_attachments as $att): ?>
                                ...
                                <tr><th class="bg-light">Tổng thanh toán:</th><td><strong class="text-success fs-5"><?php echo number_format($grand_total_vnd, 0, ',', '.'); ?> VND</strong></td></tr>
                                <tr><th class="bg-light">Phí nước ngoài:</th><td><?php echo count($foreign_charges); ?> khoản</td></tr>
                                <tr><th class="bg-light">Phí trong nước:</th><td><?php echo count($domestic_charges); ?> khoản</td></tr>
                                ...
                                <tr><th class="bg-light">Tỷ giá:</th><td>USD = <?php echo number_format($usd_rate, 0, ',', '.'); ?> | EUR = <?php echo number_format($eur_rate, 0, ',', '.'); ?></td></tr>
                            </table>
                        </div>
                    </div>
                </div>
