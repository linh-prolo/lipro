        ...
        try {
            ...
            if (empty($rows)) {
                ...
                foreach ($rows as $index => $row) {
                    ...
                    $status         = isset($row[7]) ? strtolower(trim($row[7])) : 'active';

                    // Bỏ qua dòng trống
                    if (empty($company_name) && empty($short_name)) {
                        $skipped++;
                        continue;
                    }
                    ...
                    if (empty($company_name)) {
                        $errors[] = "Dòng $row_number: Tên công ty không được để trống";
                        continue;
                    }
                    if (empty($short_name)) {
                    ...
                    // Kiểm tra đã tồn tại chưa theo short_name
                    $stmt_check = $conn->prepare("SELECT id FROM customers WHERE short_name = ?");
                    $stmt_check->bind_param("s", $short_name);
                    $stmt_check->execute();
                    $existing = $stmt_check->get_result()->fetch_assoc();
                    $stmt_check->close();

                    // Nếu không tìm thấy, thử tìm theo tax_code
                    if (!$existing && !empty($tax_code)) {
                        $stmt_check2 = $conn->prepare("SELECT id FROM customers WHERE tax_code = ? AND tax_code != ''");
                    ...
                    if ($existing) {
                        // UPDATE
                        $stmt = $conn->prepare("UPDATE customers SET company_name=?, short_name=?, tax_code=?, address=?, phone=?, email=?, contact_person=?, status=? WHERE id=?");
                        $stmt->bind_param("ssssssssi", $company_name, $short_name, $tax_code, $address, $phone, $email, $contact_person, $status, $existing['id']);
                        if ($stmt->execute()) {
                            $updated++;
                        } else {
                        ...
                        // INSERT
                        $stmt = $conn->prepare("INSERT INTO customers (company_name, short_name, tax_code, address, phone, email, contact_person, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                        $stmt->bind_param("ssssssssi", $company_name, $short_name, $tax_code, $address, $phone, $email, $contact_person, $status, $_SESSION['user_id']);
                        if ($stmt->execute()) {
                            $imported++;
                        } else {
...
<head>
    ...
    <title>Import Khách hàng - Forwarder System</title>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="index.php">Khách hàng</a></li>
            ...
            <div class="col-md-8 offset-md-2">
                <div class="card">
                    <div class="card-header bg-success text-white">
                        <h5 class="mb-0"><i class="bi bi-file-earmark-excel"></i> Import Khách hàng từ Excel</h5>
                    </div>
                    <div class="card-body">
                        <?php if ($error): ?>
                        ...
                        <div class="alert alert-info mb-4">
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm table-bordered mb-0 bg-white">
                                    ...
                                    <tbody>
                                        <tr class="small">
                                            <td>Tên công ty <span class="text-danger">*</span></td>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-sm mb-0">
                                <thead class="table-primary text-center">
                                    <tr>
                                        <th>A<br><small>Tên công ty</small></th>
                                        ...
                                        <th>G<br><small>Người liên hệ</small></th>
                                        <th>H<br><small>Trạng thái</small></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr class="table-warning text-center">
                                        <td colspan="8"><em>Dòng 1: Tiêu đề (bỏ qua khi import)</em></td>
                                    </tr>
                                    <tr>
